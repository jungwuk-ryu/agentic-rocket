...
function validSamples(samples, policy) {
  return (
    Array.isArray(samples) &&
    samples.length >= policy.minimumSamplesPerSide &&
...
export function evaluatePairedVerification(runs, policy = VERIFICATION_POLICY) {
  if (!Array.isArray(runs) || runs.length !== 3) {
    return {
      ...
      reason: "Three sandbox results are required.",
      runs: [],
    };
  }
  ...
  const evaluated = runs.map((run) => {
    ...
    if (
      ...
      run.outputPassed === false
    ) {
      return {
        ...run,
        valid: false,
        outcome: "correctness-failed",
        improvementPct: null,
      };
    }
    if (
      run.testsPassed !== true ||
      run.outputPassed !== true ||
      !validSamples(baselineSamples, policy) ||
      !validSamples(candidateSamples, policy)
    ) {
      return {
        ...run,
        valid: false,
        outcome: "insufficient-samples",
        improvementPct: null,
      };
    }
    ...
    const minimumMeaningfulImprovement = Math.max(
      ...
      Math.max(baselineCvPct, candidateCvPct, policy.calibrationCvPct || 0) *
        policy.noiseMultiplier,
    );
    const valid =
      baselineCvPct <= policy.maxBaselineCoefficientOfVariationPct &&
      candidateCvPct <= policy.maxCandidateCoefficientOfVariationPct &&
      (policy.calibrationCvPct || 0) <=
        policy.maxBaselineCoefficientOfVariationPct;
    return {
      ...run,
      baselineMs,
      candidateMs,
      ...
      outcome: !valid
        ? "noisy"
        : improvementPct >= minimumMeaningfulImprovement
          ? "improved"
          : improvementPct < 0
            ? "regressed"
            : "below-threshold",
  ...
  const aggregateImprovementPct = average(
    evaluated
      .filter((run) => Number.isFinite(run.improvementPct))
      .map((run) => run.improvementPct),
  );
  if (evaluated.some((run) => run.outcome === "correctness-failed")) {
    return {
      verdict: "rejected",
      reason: "Correctness checks failed in at least one sandbox.",
      aggregateImprovementPct,
      runs: evaluated,
    };
  }
  if (evaluated.every((run) => run.outcome === "improved")) {
    return {
      verdict: "verified",
      reason:
        "All three paired sandbox measurements exceeded their fixed noise-aware threshold.",
      aggregateImprovementPct,
      runs: evaluated,
    };
  }
  if (
    evaluated.every(
      (run) =>
        run.valid && ["regressed", "below-threshold"].includes(run.outcome),
    )
  ) {
    return {
      verdict: "rejected",
      reason:
        "The candidate regressed or did not reach the pre-declared adoption threshold.",
      aggregateImprovementPct,
      runs: evaluated,
    };
  }
  return {
    verdict: "inconclusive",
    reason:
      "The measurements were incomplete, noisy, or inconsistent across sandboxes.",
    aggregateImprovementPct,
    runs: evaluated,
  };
}
