...
export class FileSessionStore {
  ...
  async init() {
    ...
    for (const file of await readdir(this.sessionsDirectory)) {
      if (/^[a-f0-9-]{36}\.json$/i.test(file))
        await this.get(file.slice(0, -5));
    }
  }
  ...
  async create({ repositoryUrl, goal, owner }) {
    const id = randomUUID();
  ...
  async checkpoint(id, reason) {
    return this.mutate(id, async (session) => {
      session.checkpoints.push({
        ...
        failures: session.events
          .filter((event) => /error|failure/.test(event.type))
          .slice(-8),
      });
  ...
  async writeArtifact(sessionId, filename, content, contentType) {
    ...
    await this.mutate(sessionId, async (session) => {
  ...
  async getArtifact(sessionId, artifactId) {
    ...
    return (
      session?.artifacts.find((artifact) => artifact.id === artifactId) || null
    );
  }
  ...
  async readArtifact(sessionId, artifactId) {
    ...
    if (createHash("sha256").update(data).digest("hex") !== artifact.hash)
